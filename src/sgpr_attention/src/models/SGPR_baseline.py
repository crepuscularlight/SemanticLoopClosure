...
from sgpr_attention.src.models.components.layers_batch import MultiheadAttentionModule,SimpleAttentionModule,AttentionModule, TensorNetworkModule,DiffDiffTensorNetworkModule,SumDiffDiffTensorNetworkModule
...
class SGPR_Baseline(nn.Module):
    ...
    def dgcnn_conv_pass(self, x):
        ...
        x = torch.cat((xyz, sem), dim=1) #[B,64,50]
        # origin = torch.cat((xyz, sem), dim=1)#[B,64,50]

        # origin=self.pooling_layer(origin.permute(0,2,1)) #[B,50,32]
