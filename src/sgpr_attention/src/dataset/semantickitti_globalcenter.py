...
from sgpr_attention.src.dataset.utils import load_paires, process_pair_global_center
...
class SemanticKittiGlobalCenter(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = process_pair_global_center(self.graphs[idx])
        ...
        xyz_1 = np.expand_dims(data["centers_1"], axis=0)
        xyz_2 = np.expand_dims(data["centers_2"], axis=0)

        # augment data
        if self.mode == "train":
            xyz_1 = self.point_cloud_transform(xyz_1)
            xyz_2 = self.point_cloud_transform(xyz_2)

            pcn_features_1 = self.feature_transform(pcn_features_1)  # 50x1024
