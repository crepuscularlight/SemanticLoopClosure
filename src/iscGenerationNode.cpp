...
// std::queue<iscloam::LoopInfo::ConstPtr> loopBuf;
...
// void loopCallback(const iscloam::LoopInfo::ConstPtr& msg)
// {
//     mutex_lock.lock();
//     loopBuf.push(msg);
//     mutex_lock.unlock();
...
void loop_closure_detection(){
    while(1){
        if(!pointCloudBuf.empty() && !odometryBuf.empty()){
            ...
            odometryBuf.pop();
            pointCloudBuf.pop();  
            // iscloam::LoopInfo loop_in=*(loopBuf.front());
            // loopBuf.pop();
            mutex_lock.unlock();
            ...
            for(int i=0;i<(int)iscGeneration.matched_frame_id.size();i++)
                ...
                // std::cout<<"loop matched id:"<<loop.matched_id.size()<<std::endl;
                }
            // std::cout << "loop pub " << pointcloud_time << std::endl;
            ...
            // loop.current_id=loop_in.current_id;
            // loop.matched_id=loop_in.matched_id;
            // loop_info_pub.publish(loop);
        }
...
int main(int argc, char **argv)
{
    ...
    // ros::Subscriber loop_sub=nh.subscribe<iscloam::LoopInfo>("/loop_closure_c",10,loopCallback);
