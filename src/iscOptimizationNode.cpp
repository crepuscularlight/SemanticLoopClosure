...
std::queue<nav_msgs::Path::ConstPtr> gtBuf;
...
void global_optimization(){
    while(1){
        if(!loopInfoBuf.empty()&& !pointCloudSurfBuf.empty() && !pointCloudEdgeBuf.empty() && !odometryBuf.empty()){
            ...
            pcl::PointCloud<pcl::PointXYZL>::Ptr pointcloud_edge_in(new pcl::PointCloud<pcl::PointXYZL>());
            pcl::PointCloud<pcl::PointXYZL>::Ptr pointcloud_surf_in(new pcl::PointCloud<pcl::PointXYZL>());
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber loop_sub = nh.subscribe<iscloam::LoopInfo>("/loop_closure_python", 10, loopClosureCallback);
