...
void LaserMappingClass::resetMap(std::vector<Eigen::Isometry3d>& path){
	...
	for(int i=0;i<path.size();i++){
		...
		{
			pcl::PointXYZL point_temp = transformed_pc->points[j];
...
void LaserMappingClass::updateCurrentPointsToMap(const pcl::PointCloud<pcl::PointXYZL>::Ptr& pc_in, const Eigen::Isometry3d& pose_current){
...
pcl::PointCloud<pcl::PointXYZL>::Ptr LaserMappingClass::getMap(void){
	pcl::PointCloud<pcl::PointXYZL>::Ptr laserCloudMap = pcl::PointCloud<pcl::PointXYZL>::Ptr(new  pcl::PointCloud<pcl::PointXYZL>());
	for (int i = 0; i < map_width; i++){
		for (int j = 0; j < map_height; j++){
			for (int k = 0; k < map_depth; k++){
				if(map[i][j][k]!=NULL){
